...
#data_dir = os.path.abspath(os.path.join(current_dir, os.path.join('cd..')))
...
fig = plt.figure(figsize=(13,6),facecolor='#c9c9c9',num='File name : %s' %latest_file)
...
for cycle in range(1,totalitems+1):
    ...
    if cycle == 2:
        ...
        spl = 1 + count
        thd = 2 + count
        id1 = newdata['id'][8 + count]
        id2 = newdata['id'][9 + count]
        while spl < 45 + count:
            newdata[id1].append(result[spl])
            newdata['%s_lowlim' %id1].append(lowlim[spl])
            newdata['%s_uplim' %id1].append(uplim[spl])
...
ax1 = plt.subplot(2,2,1,facecolor='#efefef')
plt.ylim(-50,-10)
style(ax1)
...
ax1a = plt.subplot(2,2,2,facecolor='#f2f2f2')
plt.ylim(0,20)
style(ax1a)
...
if 'df2_fr' and 'df2_thd' in locals():
    ...
    ax2 = plt.subplot(2,2,3,facecolor='#f2f2f2')
    plt.ylim(80,140)
    style(ax2)
    ...
    plt.tick_params(axis='x', bottom=True, left=False, labelbottom=True)


    ax2a = plt.subplot(2,2,4,facecolor='#f2f2f2')
    plt.ylim(0,20)
    style(ax2a)
